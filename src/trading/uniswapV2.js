...
export class UniswapV2 {
  ...
  async sellToken(tokenAddress, tokenAmount, options = {}) {
    try {
      ...
      console.log(`💰 Selling ${ethers.formatUnits(amountIn, decimals)} tokens...`);
      ...
      console.log(`🔍 Amount to sell: ${ethers.formatUnits(amountIn, decimals)}`);
      ...
      if (allowance < amountIn) {
        ...
        try {
          ...
          for (let i = 0; i < 10; i++) {
            ...
            console.log(`Checking allowance... ${ethers.formatUnits(newAllowance, decimals)}`);
      ...
      try {
        const amountsOut = await this.routerContract.getAmountsOut(amountIn, path);
        console.log(`✅ Expected output: ${ethers.formatEther(amountsOut[1])} ETH`);
