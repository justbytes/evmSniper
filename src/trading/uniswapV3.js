...
export const v3GetPrice = async () => {
  try {
    ...
    this.dodoEgg.baseTokenDecimal = await this.getTokenDecimals(this.dodoEgg.baseTokenAddress);

    this.dodoEgg.newTokenDecimal = await this.getTokenDecimals(this.dodoEgg.newTokenAddress);
...
export const v3ProcessPriceMovement = async log => {
  ...
  const { sender, recipient, amount0, amount1, sqrtPriceX96, liquidity, tick } = decodedLog.args;
